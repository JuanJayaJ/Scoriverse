...
# -------------------------------------------------------------------
# Test for extract_additional_params (including Negative Binomial)
# -------------------------------------------------------------------

test_that("extract_additional_params extracts sigma correctly for lm models", {
  data(mtcars)
...
# Skip NB test on non-integer response data
test_that("extract_additional_params extracts phi for Negative Binomial models", {
  skip("Skipping NB test: mtcars mpg is non-integer; use appropriate count data instead.")
  # An example NB model with integer response (if available):
  # data(countdata)
  # model_nb <- glm.nb(count ~ x, data = countdata)
  # params_nb <- extract_additional_params(model_nb)
  # expect_true(is.list(params_nb))
  # expect_true(!is.null(params_nb$phi))
})
...
test_that("wrap_predict returns numeric predictions for an lm model", {
  ...
  # Check additional parameters (like sigma) attached
...
y_sc       <- c(2, 4, 6)               # True values
pred_mean  <- c(2.1, 3.9, 6.2)           # Predicted means
pred_sd    <- c(1, 1, 1)                 # Standard deviations
set.seed(123)
pred_matrix <- matrix(rnorm(300, mean = 5, sd = 1), nrow = 3)  # Prediction matrix for CRPS
...
test_that("wrap_scoring returns correct outputs for each metric", {
  ...
  # For Brier: Create dummy binary outcomes and normalize predicted probabilities.
  pred_prob <- preds
...
# -------------------------------------------------------------------
# Test for Main Workflow (scoriverse_main.R)
# -------------------------------------------------------------------

test_that("run_scoriverse end-to-end with count data", {
  set.seed(123)
  n <- 50
  ...
  # For a Poisson model, we use a proxy for pred_sd (say, sqrt(lambda))
  lambda_hat <- predict(model, newdata = count_data, type = "response")
  # Use sqrt(lambda) as a pseudo standard deviation
  result <- run_scoriverse(model, new_data = count_data, y_true = count_data$count,
                                 score_metrics = c("crps", "log_score"),
                                 visualize = FALSE, pred_sd = sqrt(lambda_hat))
  expect_true(is.list(result))
  expect_true("predictions" %in% names(result))
  expect_true("scores" %in% names(result))
  expect_equal(length(result$predictions), nrow(count_data))
})
