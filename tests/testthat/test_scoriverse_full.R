...
test_that("extract_additional_params works correctly", {
...
test_that("wrap_predict and prepare_model_for_prediction work", {
...
# -------------------------------------------------------------------
# Test: extract_predictions (with and without sampling)
# -------------------------------------------------------------------

test_that("extract_predictions returns point estimates correctly", {
...
test_that("extract_predictions returns outcome-scale samples for GLM", {
  set.seed(42)
...
# -------------------------------------------------------------------
# Test: GAM outcome-scale prediction (manual + gratia)
# -------------------------------------------------------------------

test_that("GAM predictions return expected structure (manual sampling)", {
...
if (requireNamespace("gratia", quietly = TRUE) && requireNamespace("tidyr", quietly = TRUE)) {
  test_that("GAM predictions with gratia sampling return expected structure", {
    gam_model <- mgcv::gam(mpg ~ s(wt), data = mtcars, family = gaussian)
    draws <- extract_predictions(gam_model, new_data = mtcars, return_samples = TRUE, n_samples = 10, use_gratia = TRUE)
    expect_true(is.matrix(draws))
    expect_equal(ncol(draws), nrow(mtcars))
  })
}
...
# -------------------------------------------------------------------
# Test: wrap_scoring and scoring functions
# -------------------------------------------------------------------

test_that("wrap_scoring computes correct scores or errors", {
  model <- glm(mpg ~ wt + hp, data = mtcars)
  preds <- wrap_predict(model, new_data = mtcars)
  sigma <- attr(preds, "additional_params")$sigma

  expect_error(wrap_scoring("crps", mtcars$mpg, preds))
...
# -------------------------------------------------------------------
# Test: run_scoriverse pipeline with full arguments
# -------------------------------------------------------------------

test_that("run_scoriverse executes pipeline correctly with full arguments", {
  model <- glm(mpg ~ wt + hp, data = mtcars)
  preds <- wrap_predict(model, new_data = mtcars)
  params <- attr(preds, "additional_params")
  sigma <- rep(params$sigma, length(preds))  # ensure vector format

  # Rescale preds to 0-1 for binary Brier
  prob_pred <- (preds - min(preds)) / (max(preds) - min(preds))
